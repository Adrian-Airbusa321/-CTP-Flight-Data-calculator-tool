...
CLIMB_BURN_FACTOR        = 1.60       # × cruise burn
...
# ---------------------------------------------------------------------------
# Data classes (Luckily not from Indian Scammers)
# ---------------------------------------------------------------------------
@dataclass
class Aircraft:
    id:              str
...
# ---------------------------------------------------------------------------
# Loading (Windows also always loading)
# ---------------------------------------------------------------------------
def _is_icao(v) -> bool:
...
def format_report(c: Calculation) -> str:
    ...
    out.append(line)
    out.append("  RESERVES")
    out.append(f"   Contingency  (5 % trip)                       {fmt_kg(c.contingency_kg)} kg")
    out.append(f"   Alternate    ({c.alternate_nm:.0f} nm)                      {fmt_kg(c.alternate_kg)} kg")
    out.append(f"   Final reserve (30 min)                        {fmt_kg(c.final_reserve_kg)} kg")
...
def interactive(aircraft: Dict[str, Aircraft],
    ...
    c = calculate(chosen, route, payload_kg=payload, wind_component_kt=wind)
    print()
